...
from utils import to_var, sloppy_log_add
...
class SentenceVAE(nn.Module):
    ...
    def get_sentence_log_probability(self, sentence, n=1, z=None):
...
#         running_seqs = torch.arange(0, batch_size, out=self.tensor()).long() # idx of still generating sequences with respect to current loop
        t = 0
        logp = 0.0
        while(t < (len(sentence) - 1)):

            input_sequence = to_var(torch.Tensor(batch_size).fill_(sentence[t]).long())
            ...
            output, hidden = self.decoder_rnn(input_embedding, hidden)
            logits = self.outputs2vocab(output)  
            logp += nn.functional.log_softmax(logits, dim=-1).squeeze()[sentence[t+1]].data.cpu().numpy()[0]
